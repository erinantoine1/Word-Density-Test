...
const Tutorial = (props) => {
  ...
  const fakeClickedReady = (e) => {
    props.setDisplayTutorialReadyButton(false);
    props.setArtistInputBorder('solid 2px #001528')
    props.setTitleInputBorder('solid 2px #001528')
    e.preventDefault();
  }
  const readyUp = (e) => {
    //props.displayTutorialReadyButton
  ...
  return (
    <div style={{display: 'flex', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', alignContent: 'center', height: '100%'}}>
      <div style={{height: '80.0%', width: '100%', display:'flex', flexWrap: 'wrap', justifyContent:'center', alignItems: 'center', marginLeft: '35px'}}>
        <div style={{position: 'relative', border: 'solid', height: '600px', width: '850px', backgroundColor: '#001528', display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: '10px', marginTop: '-130px', marginLeft: '100px'}} ref={props.ref11}>
          <div style={{position: 'relative', border: 'dashed #1776ff 3px', height: '585px', width: '835px', backgroundColor: '#001528', display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: '10px'}}>
              <div style={{position: 'relative', border: 'solid 1px #fafafa', height: '570px', width: '820px', backgroundColor: '#001528', display: 'flex', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', borderRadius: '10px', color: '#fafafa'}}>
                <div style={props.beforeStart && props.displayTutorialReadyButton ? {visibility: "visible", position: 'absolute', top: '23%', color: '#fafafa', fontSize: '75px'} : {visibility: "hidden"}}>
                  Click to Start
                </div>
                <div style={props.beforeStart && !props.displayTutorialReadyButton ? {visibility: "hidden"} : {visibility: "visible", position: 'absolute', top: '23%', color: '#fafafa', fontSize: '75px'}}>
                  Click to Start
                </div>
                <div style={props.beforeStart && props.displayTutorialReadyButton ? {visibility: "visible", position: 'absolute', top: '43%'} : {visibility: "hidden"}}>
                  <img src={downArrows} alt=""/><img src={downArrows} style={{marginLeft: '60px'}} alt=""/><img src={downArrows} style={{marginLeft: '60px'}} alt=""/>
                </div>
                <div style={props.beforeStart && !props.displayTutorialReadyButton ? {visibility: "hidden"} : {visibility: "visible", position: 'absolute', top: '43%'}}>
                  <img src={downArrows} alt=""/><img src={downArrows} style={{marginLeft: '60px'}} alt=""/><img src={downArrows} style={{marginLeft: '60px'}} alt=""/>
                </div>
                <div style={props.displayTutorialFeedback ? {width: '100%', display: 'inline-flex', justifyContent: 'center', fontSize: '60px', alignItems: 'center', position: 'absolute', top: '50px'} : {display: 'none'}}>
                  {props.statusMessage}
                </div>
                <div style={{visibility: "visible", position: 'absolute', top: '28%', display: 'flex', flexWrap: 'wrap', justifyContent: 'center', width:
              '100%'}}>
                  <div style={props.completedTitle ? {display: 'flex', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', marginTop: '5px'}: {display: 'none'}}>
                    <div>
                ...
                <Button type="primary" style={props.beforeStart && props.displayTutorialReadyButton ? {visibility: "visible", height: '75px', width: '400px', fontSize: '28px', padding: '-30%', position: 'absolute', top: '58%', animation:'shake 0.82s cubic-bezier(.36, .07, .19, .97) both infinite'} : {visibility: 'hidden'}}  onClick={(e) => {fakeClickedReady(e)}} ref={props.ref9}>Ready!</Button>
                {/* <Button type="primary" style={props.beforeStart && !props.displayTutorialReadyButton ? {visibility: 'hidden'} : {visibility: "visible", height: '75px', width: '400px', fontSize: '28px', padding: '-30%', position: 'absolute', top: '58%', animation:'shake 0.82s cubic-bezier(.36, .07, .19, .97) both infinite'}}  onClick={(e) => {readyUp(e)}} ref={props.ref9}>Ready!</Button> */}
                <Button type="primary" style={props.questionComplete ? {visibility: "visible", height: '75px', width: '400px', fontSize: '28px', position: 'absolute', top: '425px'} : {visibility: 'hidden'}} ref={props.ref12}>Ready!</Button>
              </div>
            </div>
        </div>
      </div>
      <div style={{height: '30.0%', width: '400px', fontSize: '80px', marginLeft: '130px', marginTop: '-120px'}} ref={props.ref10}>
        <Form>
          ...
          <Button type="dashed" htmlType="button" style={{margin: '10px', height: '50px', width: '140px', fontSize: '24px'}}>Give Up</Button>
          </Form.Item>
        </Form>
      </div>

      <Tour open={props.tourOpen} onClose={() => props.closeTutorial()} steps={props.steps} />
    </div>
  );
};
